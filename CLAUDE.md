...
### 1. 搜索防抖缩短到 50ms（Performance）
**文件：** `HistoryWindow.xaml.cs`

调整：搜索框输入防抖由 `300ms` 改为 `50ms`，减少键入后结果更新的固定等待时间。

### 2. 预览读取支持真正取消（Performance）
**文件：** `HistoryWindow.xaml.cs`、`AppController.cs`

调整：`HistoryWindow` 注入的 payload loader 改为透传 `CancellationToken`，切换选中项或继续输入搜索时，旧的预览读取任务会被取消，而不是继续占用 IO。

### 3. 搜索与预览 IO 解耦（Performance）
**文件：** `SqliteClipRepository.cs`

调整：仓储层将写入串行锁与搜索/预览读取分离。搜索查询不再与 payload 文件读取共用同一把锁，避免右侧预览加载拖慢左侧搜索结果刷新。

### 4. 连续中文查询改走子串匹配（Search Fix）
**文件：** `HistoryWindow.xaml.cs`、`SearchNormalizer.cs`

调整：遇到连续 CJK 文本查询时，优先使用内存子串匹配，不再依赖 SQLite FTS5 的前缀 token 匹配去命中中文中间位置；同时在 FTS 返回空结果时追加内存搜索兜底。
